...
RSpec.describe "get_all_customers" do
  it 'returns all customers in database' do
    ...
    post '/graphql', params: { query: all_locations}

    expect(response).to be_successful
    response_body = JSON.parse(response.body, symbolize_names: true)
    data = response_body[:data]
    expect(data).to have_key(:allLocations)
    expect(data[:allLocations].first).to have_key(:id)
    expect(data[:allLocations].first).to have_key(:numberOfBins)
    expect(data[:allLocations].first).to have_key(:streetAddress)
    expect(data[:allLocations].first).to have_key(:city)
    expect(data[:allLocations].first).to have_key(:state)
    expect(data[:allLocations].first).to have_key(:pickupDay)
    expect(data[:allLocations].first).to have_key(:customerId)
    expect(data[:allLocations].first).to have_key(:pickedUp)
  end
